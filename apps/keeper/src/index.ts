      ...
      try {
        ...
        const challenges = bounties.map((b) => {
          ...
          return {
            ...
            fee: "0",
            nudgeCount: 0, // Real nudges from humans (not creature's own iterations)
            iterations: hunt?.iteration || 0, // Creature's own submission attempts
            platformSubmissions: b.submissionCount || 0, // Total on Clankonomy
    ...
    if (challengeMatch && req.method === "GET") {
      ...
      try {
        ...
        res.end(JSON.stringify({
          ...
          fee: "0",
          nudgeCount: 0,
          iterations: hunt?.iteration || 0,
          platformSubmissions: bounty.submissionCount || 0,
          categorySlug: catSlug,
          evalType: bounty.evalType,
          evalScript: bounty.evalScript,
          evalSummary: bounty.evalSummary,
          fileType: bounty.allowedFileTypes?.[0] || bounty.fileType,
